...
const uniqueColumns = (excelColumns: string[], modernColumns: string[]) => [
	...excelColumns,
	...modernColumns.filter((column) => !excelColumns.includes(column)),
];

const rowByIndex = (rows: IterationRow[], index: number) => rows[index];

const numericDelta = (
	excelValue: number | string | undefined,
	modernValue: number | string | undefined,
) =>
	typeof excelValue === "number" && typeof modernValue === "number"
		? Math.abs(excelValue - modernValue)
		: undefined;
...
export function CompareTable({ excelResult, modernResult }: CompareTableProps) {
	...
	const rowCount = Math.max(
		excelResult.iterations.length,
		modernResult.iterations.length,
	);
				...
				<div className="flex items-center gap-2">
					...
					<div>
						<h3 className="text-sm font-semibold text-zinc-100">
							Comparacion por celda
						</h3>
						<p className="text-xs text-zinc-500">
							Cada columna muestra el valor Excel y el moderno apilados para la
							misma iteracion.
						</p>
					</div>
				</div>
			...
			<div className="max-h-[calc(100vh-420px)] max-w-full overflow-auto overscroll-contain">
				<table className="w-max min-w-full border-collapse text-left text-sm">
					<thead className="sticky top-0 z-10 bg-black text-xs uppercase tracking-wide text-zinc-500">
						<tr>
							<th className="w-28 border-b border-zinc-800 px-3 py-2">
								Iteracion
							</th>
							{columns.map((column) => (
								<th
									key={column}
									className="min-w-44 border-b border-zinc-800 px-3 py-2"
								>
									{column}
								</th>
							))}
								...
								<td className="px-3 py-3 align-top font-mono text-zinc-500">
									...
									{row.excelRow?.iteration !== row.modernRow?.iteration && (
										<div className="mt-1 text-[11px] text-zinc-600">
											E:{row.excelRow?.iteration ?? "-"} / M:
											{row.modernRow?.iteration ?? "-"}
										</div>
									)}
								</td>
								{columns.map((column) => {
									...
									return (
										<td key={column} className="px-3 py-3 align-top">
											<div className="space-y-1 rounded border border-zinc-800 bg-black px-2.5 py-2 font-mono">
												...
												/>
												<div
													className={`border-t border-zinc-800 pt-1 text-[11px] ${deltaTone(delta)}`}
												>
													{formatNumber(delta) || "-"}
												</div>
											</div>
...
function ValueLine({
	...
	return (
		<div className={`min-w-0 break-words text-xs ${className}`}>
			{formatNumber(value) || "-"}
		</div>
	);
}
