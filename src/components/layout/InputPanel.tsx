...
export function InputPanel({ kind, method, mode, input, update, onReset }: InputPanelProps) {
  return (
    <section className="rounded-md border border-zinc-800 bg-zinc-950 p-4 shadow-xl shadow-black/20">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">{modeCopy[mode].description}</p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <NumberField label="Tolerancia" value={input.tolerance} onChange={(tolerance) => update({ tolerance })} />
        <NumberField label="Max iteraciones" step="1" value={input.maxIterations} onChange={(maxIterations) => update({ maxIterations: Math.max(1, Math.round(maxIterations)) })} />
      </div>
    </section>
  )
}
