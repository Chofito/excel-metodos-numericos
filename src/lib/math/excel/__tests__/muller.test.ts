...
import { solveExcelMuller } from "../index";

describe("muller Excel 1:1", () => {
	test("case-1: f(x)=x^3-x-2, x0=1, x1=1.5, x2=2, tol=0.001", async () => {
		const f = await Bun.file(
			`${import.meta.dir}/fixtures/muller-1.json`,
		).json();
		if (!f.rows.length) return;
		const result = solveExcelMuller({ ...defaultInput, ...f.input });
