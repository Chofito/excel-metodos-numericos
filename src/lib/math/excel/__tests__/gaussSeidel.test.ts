...
import { solveExcelGaussSeidel } from "../index";

describe("gauss-seidel Excel 1:1", () => {
	test("case-1: 3x3 sistema diagonalmente dominante, tol=0.001", async () => {
		const f = await Bun.file(
			`${import.meta.dir}/fixtures/gaussSeidel-1.json`,
		).json();
		if (!f.rows.length) return;
		const result = solveExcelGaussSeidel({ ...defaultInput, ...f.input });
