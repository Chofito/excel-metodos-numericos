...
import { solveExcelNewton } from "../index";

describe("newton Excel 1:1", () => {
	test("case-1: f(x)=x^3-x-2, x0=1.5, tol=0.001 — derivada progresiva VBA h=0.1", async () => {
		const f = await Bun.file(
			`${import.meta.dir}/fixtures/newton-1.json`,
		).json();
		if (!f.rows.length) return;
		const result = solveExcelNewton({ ...defaultInput, ...f.input });
