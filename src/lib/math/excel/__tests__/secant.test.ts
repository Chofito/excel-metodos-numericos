...
import { solveExcelSecant } from "../index";

describe("secante Excel 1:1", () => {
	test("case-1: f(x)=x^3-x-2, x0=1, x1=2, tol=0.001", async () => {
		const f = await Bun.file(
			`${import.meta.dir}/fixtures/secant-1.json`,
		).json();
		if (!f.rows.length) return;
		const result = solveExcelSecant({ ...defaultInput, ...f.input });
