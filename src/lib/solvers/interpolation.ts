...
export function solveLagrange(
	...
	for (let i = 0; i < points.length; i += 1) {
		...
		for (let j = 0; j < points.length; j += 1) {
			if (i !== j)
				basis *= (input.targetX - points[j].x) / (points[i].x - points[j].x);
		}
	...
	return methodResult(
		...
		[
			modern
				? "Forma directa de Lagrange."
				: "Replica la evaluacion directa del modulo lagrange_AR.",
		],
		sum,
	);
}
...
export function solveDividedDifferences(
	input: NumericInput,
	modern = false,
): MethodResult {
	...
	return methodResult(
		"converged",
		`P(${input.targetX}) = ${approximation}`,
		columns,
		rows,
		[
			modern
				? "Evalua forma regresiva."
				: "Replica el valor escrito por el VBA, que evalua la forma progresiva.",
		],
		approximation,
	);
}
