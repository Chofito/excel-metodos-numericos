...
export const excelEval = (expression: string, replacements: Record<string, number>) => {
...
export const modernEval = (expression: string, scope: Record<string, number>) =>
  toNumber(evaluate(expression, { pi: Math.PI, Pi: Math.PI, e: Math.E, ...scope }))

export const scalarFunction = (input: NumericInput, modern: boolean) => (x: number) =>
  modern ? modernEval(input.formula, { x }) : excelEval(input.formula, { x })

export const fixedPointFunction = (input: NumericInput, modern: boolean) => (x: number) =>
  modern ? modernEval(input.transformFormula, { x }) : excelEval(input.transformFormula, { x })
